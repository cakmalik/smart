...
<div class="grid grid-cols-1 md:grid-cols-4 gap-2 mx-4 sm:mx-0">
    ...
    <x-card.summary title="Pendaftar" :value="$summary['new_students']" color="atlantis" />
    </Link>
    <Link href="{{ route('student.index') }}">
    <x-card.summary title="Total Santri {{ auth()->user()->gender == 'male' ? 'Laki-laki' : 'Perempuan' }}" :value="$summary['students']"
        color="danube" />
    </Link>
    <Link href="{{ route('student.new') }}">
...
<div class="grid grid-cols-1 md:grid-cols-3 gap-2 mt-3  mx-4 sm:mx-0">
    <Link href="{{ route('invoice.index') }}" class="bg-slate-200 rounded-lg p-3">
