...
<div class="">
    <button id="dropdownHoverButton{{ $id }}" data-dropdown-toggle="dropdownHover{{ $id }}" data-dropdown-trigger="hover"
        type="button"
        ...
        <span
            class="text-neutral-200 text-center text-sm group-hover:text-white  transition duration-300 ease-in-out">{{ $label }}</span>
    </button>
    <div id="dropdownHover{{ $id }}"
        class="z-10 hidden bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700">
        <ul class="py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownHoverButton{{ $id }}">
           {{ $slot }}
        </ul>
    </div>
</div>