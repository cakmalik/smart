    <div class="grid grid-cols-1 md:grid-cols-4 gap-2 mx-4 sm:mx-0">
        ...
        <Link href="{{ route('student.index') }}">
        <x-card.summary title="Santri Putri" :value="$summary['student_p_count']" color="danube" />
        </Link>
        <Link href="{{ route('invoice.index') }}">
        <x-card.summary title="Tagihan Belum dibayar" :value="$summary['mutation']" color="mantis" />
        </Link>
    </div>

    <div class="grid grid-cols-1 md:grid-cols-3 gap-2 my-3 mx-4 sm:mx-0">
        ...
        <div class="flex flex-col gap-2">
            <div class="flex justify-between items-center"><span>PSB {{ date('Y') }}</span> <span
                    class="bg-slate-500 text-white text-xs p-1 rounded font-semibold">SANTRI BARU</span></div>
            <div class="text-3xl font-bold flex justify-between items-center">
                <span class="text-xl font-light">L: <span
                        class="font-semibold">{{ $summary['psb_student_l_count'] }}</span> &nbsp; P: <span
                        class="font-semibold">{{ $summary['psb_student_p_count'] }}</span></span>
